...
int main(int argc, const char** argv)
{
    ...
    // glfw Î©îÏù∏ Î£®ÌîÑ ?ã§?ñâ, ?úà?èÑ?ö∞ close Î≤ÑÌäº?ùÑ ?àÑÎ•¥Î©¥ ?†ï?ÉÅ Ï¢ÖÎ£å
    SPDLOG_INFO("Start main loop");
    while (!glfwWindowShouldClose(window)) {
        
        // window ?óê?Ñú Î∞úÏÉù?ïò?äî ?ù¥Î≤§Ìä∏Î•? ?àòÏß?
        // ?ù¥Î≤§Ìä∏ Î∞úÏÉù ?ãú?óê, ?ûê?èô?úºÎ°? Í∑∏Ïóê ÎßûÎäî ÏΩúÎ∞± ?ï®?àòÎ•? ?ò∏Ï∂úÌï¥Ï§??ã§
        glfwPollEvents();


        // UI Î•? Îß? ?îÑ?†à?ûÑÎßàÎã§ Í∑∏Î¶∞?ã§
        ImGui_ImplGlfw_NewFrame();
        ImGui::NewFrame(); // Ïß?Í∏àÎ???Ñ∞ ?Éà ?†å?çîÎß? ?îÑ?†à?ûÑ?ù¥?ã§

       
        // ?úà?èÑ?ö∞?óê ?ì§?ñ¥?ò§?äî ?ù∏?íã?ùÑ Ï≤òÎ¶¨
        context->ProcessInput(window);


        // Context Object ?ù¥?ö© => ?ôîÎ©¥Ïóê Í∑∏Î¶¨?äî ?ûë?óÖ?ùÑ ÏßÑÌñâ?ïú?ã§
        context->Render();


        // ?î¨?ùÑ ?†å?çîÎß? ?ïú ?õÑ, UI Î•? ?†å?çîÎßÅÌïú?ã§
        ImGui::Render();    // Í∑∏Î†§?ïº ?ïò?äî ?†ïÎ≥¥Î?? Í∞??†∏?ò§Í≥?
        ImGui_ImplOpenGL3_RenderDrawData(ImGui::GetDrawData()); // Í∑∏Î¶∞?ã§


        /* 
            Double Buffering, Frame Buffer Swap

            Front buffer ??? Back Buffer Î•? ?î∞Î£? Íµ¨ÌòÑ, Î®ºÏ?? back buffer ?óê Í∑∏Î¶∞ ?ã§?ùå?óê
            Front Buffer ??? Back Buffer Î•? Î∞îÍæº?ã§
         */
        glfwSwapBuffers(window);    // ?îÑ?†à?ûÑ Î≤ÑÌçº ?ä§?ôë ÏΩîÎìú
    }
